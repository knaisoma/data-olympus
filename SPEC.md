...
**Version:** 0.4
**Date:** 2026-09-14
**Status:** Stable (0.4 ships with data-olympus 0.8.1; the format is versioned independently of the package, see section 10)
...
**Targets are stable concept IDs, never paths.** A `supersedes` / `superseded_by` / `contradicts` value must be the target document's `id` field, not a file path: paths break silently on rename (see the onboarding rename machinery in the reference implementation, which exists precisely because paths are not stable identifiers), while ids are designed to survive a move. `kb lint` warns when a value looks path-shaped (contains a `/` or ends in `.md`) and suggests the fix; a path-shaped `supersedes` or `superseded_by` value that also does not resolve to a document is an error, like any other unresolved supersession target.
...
- **Errors** (block CI): a malformed value shape (a `supersedes`/`contradicts` entry that isn't a string, or a `superseded_by` that isn't a scalar string); a `supersedes` or `superseded_by` target id that does not resolve to any document in the bundle, including a path-shaped value that does not resolve (since `0.4`: such an edge retires nothing); a document that supersedes or is superseded_by itself; a supersession cycle (following the merged `supersedes`/`superseded_by` graph, of any length: A supersedes B supersedes A, or longer). A target resolves when any concept in the bundle carries it as its `id`, whatever that document's `status`.
- **Warnings** (do not block CI by default): a dangling `contradicts` target id (not found anywhere in the bundle); an asymmetric pair (A supersedes B but B's `superseded_by` doesn't name A, in either direction); `superseded_by` set while the document's own `status` is in the in-force class (see section 8); `status: superseded` with no `superseded_by` set; a path-shaped target value that resolves, or any path-shaped `contradicts` value; two in-force documents that `contradicts` each other (in either direction).

The reference implementation's `kb lint` (`data-olympus lint`) resolves targets within the linted directory; `--resolve-root <dir>` additionally resolves them against every concept under `<dir>` (existence only, for linting a subtree), and `--unresolved-targets warn` reports an unresolved supersession target as a warning while a backlog is fixed. `data-olympus import` accepts the same two options.
...
Before `1.0`, a minor increment may also tighten validation when the looser rule let a conformant document silently fail to do what it declares. Every such tightening is recorded in this section with its migration path. From `1.0` onward, any change that makes a conformant bundle fail validation requires a major increment.
...
Version `0.3` incremented from `0.2`, because what a conformant writer produces changed: OKF v0.2 supersedes `timestamp` with `generated: { by, at }`, so the recommended content-change field is now `generated` (section 4.2). The change is backward compatible for readers: a legacy `timestamp` still satisfies the recommendation and no bundle needs rewriting. The only new finding an existing document can gain is a warning for a `generated` value it already carries in a malformed shape (for example `generated: null`), which earlier versions ignored. Bundles scaffolded by `data-olympus init` at that version declared `spec_version: "0.3"` and `okf_version: "0.2"`. A consumer that implements only OKF v0.1 and requires `timestamp` will not accept newly written documents; OKF v0.2 consumers require only `type`.

This document is at `0.4`, incrementing from `0.3`, because validation tightened: a `supersedes` or `superseded_by` target that does not resolve to a document is now a lint error instead of a warning. Such an edge was always inert (it retired nothing, and a `superseded_by` named a successor that does not exist), so the looser rule let a document silently fail to do what it declared. A bundle that linted clean at `0.3` with that warning fails at `0.4`; nothing else about reading, parsing or validating a bundle changes, and `contradicts` keeps its warning. Migration: run `data-olympus lint --unresolved-targets warn` to list the unresolved targets, fix or remove each one, then drop the flag; lint a subtree with `--resolve-root <bundle root>` so targets outside it resolve. Bundles scaffolded by `data-olympus init` declare `spec_version: "0.4"`. The reference implementation's write path now also refuses a write that introduces such a target (issue #259); like issue #114 that is an enforcement-timing change, not a format change.
