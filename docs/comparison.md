...
These numbers were measured once, at the commit recorded as `source_commit` in that receipt, under the `uv.lock` committed at that same commit. The guard verifies the receipt against **that** lock, read from git history, byte for byte. It deliberately does **not** require the current `uv.lock` to match: a later dependency update does not retroactively change a past measurement, and requiring equality would have asserted something untrue, that today's dependency set produced numbers measured before it existed. So a dependency bump is free to land, and the numbers keep pointing at the environment that actually produced them. What the guard therefore does not establish is that the benchmarks were *executed* under the recorded dependencies at all, nor anything about the integrity of the dependency set shipping today; the first needs a fresh measured run, and the second is the job of dependency and security review.
...
|---|---|---|---|---|---|
| fts-no-metadata | 0.667 | 0.270 | 1.000 | 0.345 | 306 |
| fts+description | 0.667 | 0.284 | 1.000 | 0.352 | 305 |
| fts+applies_when | 1.000 | 0.311 | 1.000 | 0.434 | 307 |
| fts+applies_when+abstain | 1.000 | 0.270 | 0.097 | 0.414 | 199 |
| bm25-baseline | 1.000 | 0.243 | 1.000 | 0.400 | 658 |
<!-- BENCH:governance END -->

What the ablation honestly shows:

- **`applies_when` helps, and more than in the old 18-doc run.** On trigger-covered intents it lifts recall from 0.667 to **1.000** (+0.333), fixing cases where the model's tool/intent term is *not* already in the doc's prose; overall recall rises to **0.434** over 0.345 for body-only FTS, at roughly half BM25's tokens. The gain is larger than the old benchmark reported because the corpus grew (30 topics, so top-5 is no longer trivial) and the trigger terms are strictly held out of the body. (All numbers here are the exact values from the generated table above.)
- **It cannot bridge true paraphrases.** On the strictly token-disjoint `paraphrase_uncovered` stratum, `applies_when` reaches only 0.311 (up just +0.041 over the 0.270 body-only FTS gets). When the query shares no token with any trigger, curated metadata alone cannot help; that is dense/semantic-retrieval territory. The optional local-embedding hybrid (measured in [`benchmarks/governance_results/embeddings/ablation.md`](../benchmarks/governance_results/embeddings/ablation.md)) lifts this stratum from 0.311 to ~0.527, which is exactly the gap it is meant to close. (These paraphrase numbers dropped from an earlier draft: an FTS-token leak in the "held-out" queries had inflated them, and fixing it is why the honest figure is lower.)
- **`description` alone barely moves recall** (+0.007 overall, 0.352 vs 0.345); the trigger list is the lever that matters.
