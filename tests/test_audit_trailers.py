...
def test_build_commit_message_includes_subject_and_all_trailers() -> None:
    msg = build_commit_message(
        subject="propose: operator/memory/inbox/2026-06-01-test.md",
        source_session="session-xyz",
        agent_identity="claude",
        confidence_original=0.92,
        operator_confirmed=False,
        proposal_type="memory",
        target_tier="operator",
        target_path="operator/memory/inbox/2026-06-01-test.md",
    )
    assert msg.startswith("propose: operator/memory/inbox/2026-06-01-test.md\n\n")
    ...
    assert "KB-Target-Tier: operator" in msg
    assert "KB-Target-Path: operator/memory/inbox/2026-06-01-test.md" in msg


def test_build_commit_message_preserves_original_confidence_after_resolve() -> None:
    """Per spec §2.6: original confidence is preserved even when a low-confidence
